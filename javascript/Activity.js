...
class Activity {
    ...
    //sets and gets different properties
    setProject(project){
        this.project=project;
    }
    getProject(){
    ...
    loadDataActivity(days,month, year){
        ...
        webSocket.addEventListener("message", function(event){
            //creates variable to get the data parsed
            let callActivities= JSON.parse(event.data);
            
            if (callActivities[0].nameTable.includes("activity")==true){
                ...
                //iterates through selection list of the activities form and deletes all its children to add updated data
                while (optionsActivity.childElementCount) {
                    optionsActivity.lastChild.remove();
                }
                ...
                callActivities.forEach(element => {
                    ...
                    newTd.textContent=`${element.assignee}-${element.project}-${element.priority}-${element.hours}`; 
    ...
    deleteActivityTable (){
        ...
        selectActivities.childNodes.forEach(element => {
            if (element.selected===true){
                ...
                //selects a row in the table a deletes it
                let tableActivities=document.getElementById(`${element.id}`);
                tableActivities.remove();
