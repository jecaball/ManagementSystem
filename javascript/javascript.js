...
deleteActivities.addEventListener("click", ()=> {
    ...
    //reöpad the database in the table
    Activities.loadDataActivity(newTable.getDays(),newTable.getMonth(),newTable.getYear());
    
}, false)

// Creates a variable that checks the size of the body of the html
let displayChanges= document.body;

//Creates a variable fpr the handling of the components of the html
let bodyMainPage = document.querySelectorAll("div.body-mainpage").item(0);

//adds an eventi listener to the windows in otder to define how to change the display according the size of the window
window.addEventListener("resize", ()=>{
    if (window.innerWidth <=1250){
        bodyMainPage.style.gridTemplateRows ="795px 795px";
        bodyMainPage.style.gridTemplateColumns ="100%";
        
    } else if (window.innerWidth>1250){
        bodyMainPage.style.gridTemplateColumns="34% 64%";
        bodyMainPage.style.gridTemplateRows="100%";
    }
},false)
